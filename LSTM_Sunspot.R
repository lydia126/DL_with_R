...
# sun_spots_sub <- sun_spots[1:624,]
p2 <- sun_spots_sub %>%
#  filter_time("1749" ~ "1800") %>%
...
# 5.1.4 LSTM Plan
# Model inputs
lag_setting  <- 120 # = nrow(df_tst), prediction of window or test set length
batch_size   <- 40   # evenly divides into the # of testing (120) and training (600) observations
train_length <- 440  # why not 600?
tsteps       <- 1    # number of lags (120) included in the training/testing set
epochs       <- 300  # to be adjusted to balance the bias/variance tradeoff.
...
sample_predictions_lstm_tbl   # here
